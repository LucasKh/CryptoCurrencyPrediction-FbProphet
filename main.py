...
# -------------------------------------------------------
st.subheader("Now let's calculate and plot the Mean Absolute Error")

y_true = df_train['y'].values
y_pred = forecast['yhat'][:-365].values
mae = mean_absolute_error(y_true, y_pred)
st.subheader(" The Mean Absolute Error is : %.3f" % mae)
mape = mae/100

st.subheader("mean absolute percentage error:%.2f" % mape)
predicted = y_pred
true = y_true
df = pd.DataFrame()
df['true'] = true
